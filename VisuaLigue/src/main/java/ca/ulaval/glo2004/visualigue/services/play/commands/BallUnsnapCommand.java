...
public class BallUnsnapCommand extends Command {
    ...
    public BallUnsnapCommand(Play play, Long time, String ballActorUUID, String playerActorUUID) {
        super(play, time);
    ...
    public Long execute() {
        ...
        oldPlayerOwnerKeyframe = play.merge(time, ballActor, BallState.getOwnerPlayerProperty(), null, new LinearKeyframeTransition());
        oldSnappedBallKeyframe = play.merge(time, playerActor, PlayerState.getSnappedBallProperty(), null, new LinearKeyframeTransition());
        return time;
    }
